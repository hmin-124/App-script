...
class DashboardService {
   ...
   * validation) + hướng dẫn ngắn. Vị trí luôn khớp với `Config.FILTER_VALUE_
   ...
   * @param {number} row - Dòng để vẽ filter (luôn bằng Config.FILTER_VALUE_POSITION.row).
   * @returns {number} Dòng kế tiếp còn trống.
   */
  renderMonthFilter(selectedMonth, availableMonths, row) {
    try {
      const layout = Config.LAYOUT;
      const colors = Config.COLORS;
      const position = Config.FILTER_VALUE_POSITION;
      ...
      const months =
        availableMonths && availableMonths.length > 0 ? availableMonths : Utils.generateYearMonths(new Date().getFullYear());
   ...
   * trên vị trí cố định `Config.FILTER_VALUE_POSITION`. Dùng bởi `onEdit()`
