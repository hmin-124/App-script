 ...
 * render UI. Nhận vào mảng record đã lọc theo tháng (từ DataService), trả về
...
class ReportService {
  ...
  static calculateSummary(records) {
    try {
      ...
      const roi = Utils.safeDivide(profit, totals.cost) * 100;
      const roas = Utils.safeDivide(totals.revenue, totals.cost);
    ...
    try {
      ...
      records.forEach((record) => {
        ...
        if (!groups.has(dayKey)) {
          groups.set(dayKey, { date: record.date, dateLabel: Utils.toDayLabel(record.date), cost: 0, revenue: 0 });
        }
   ...
   * @param {number} [topN=Config.TOP_N]
   * @returns {{topMembers:Array<Object>, topBrands:Array<Object>}}
   */
  static getTopPerformers(memberReport, brandReport, topN = Config.TOP_N) {
    try {
      return {
  ...
  static _groupBy(records, keySelector) {
    ...
    return Array.from(groups.values())
      .map((bucket) => {
        ...
        const roi = Utils.safeDivide(profit, bucket.cost) * 100;
