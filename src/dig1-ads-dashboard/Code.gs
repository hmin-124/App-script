 ...
 *      ngược lại vào CONFIG (qua DataService.setSelectedMonth) để CONFIG
 *      luôn là nơi lưu trữ chính thức của lựa chọn hiện tại.
...
function onEdit(e) {
  try {
    ...
    if (editedSheet.getName() === Config.SHEET_NAMES.CONFIG) {
      const selectedMonthCell = dataService.getConfigValueCell(Config.CONFIG_KEYS.SELECTED_MONTH);
      if (!selectedMonthCell) return;

      if (Utils.isCellWithinRange(selectedMonthCell.getRow(), selectedMonthCell.getColumn(), e.range)) {
    ...
    if (editedSheet.getName() === Config.SHEET_NAMES.DASHBOARD) {
      const filterRange = DashboardService.getMonthFilterValueRange(editedSheet);
      if (!Utils.isCellWithinRange(filterRange.getRow(), filterRange.getColumn(), e.range)) return;

      const newMonth = String(e.range.getValue() || '').trim();
      if (!Utils.isValidYearMonth(newMonth)) {
