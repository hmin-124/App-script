 ...
 * Tách riêng để test độc lập và dùng lại ở DashboardDataService/ReportService.
 */
...
class DashboardUtils {
  ...
  static formatCurrency(amount) {
    try {
      ...
      return `${DashboardConfig.CURRENCY_SYMBOL}${formatted}`;
    } catch (error) {
      console.error(`DashboardUtils.formatCurrency: ${error.message}`);
  ...
  static formatPercent(value, decimals = 2) {
    try {
      ...
      console.error(`DashboardUtils.formatPercent: ${error.message}`);
      return String(value);
    }
  }
  ...
  static formatNumber(value, decimals = 0) {
    try {
      ...
      console.error(`DashboardUtils.formatNumber: ${error.message}`);
      return String(value);
    }
  }
  ...
  static parseNumber(value) {
    try {
      ...
      console.error(`DashboardUtils.parseNumber: ${error.message}`);
      return 0;
    }
  }
  ...
  static toDate(value) {
    try {
      ...
      console.error(`DashboardUtils.toDate: ${error.message}`);
      return null;
    }
  }
  ...
  static toYearMonth(date) {
    try {
      ...
      console.error(`DashboardUtils.toYearMonth: ${error.message}`);
      return '';
    }
  }
  ...
  static toDayLabel(date) {
    try {
      ...
      console.error(`DashboardUtils.toDayLabel: ${error.message}`);
      return '';
    }
  }
  ...
  static formatMonthLabel(yearMonth) {
    try {
      ...
      console.error(`DashboardUtils.formatMonthLabel: ${error.message}`);
  ...
  static isCellWithinRange(row, column, range) {
    try {
      ...
      console.error(`DashboardUtils.isCellWithinRange: ${error.message}`);
      return false;
    }
  }
  ...
  static buildSampleAdsRows() {
    try {
      ...
      console.error(`DashboardUtils.buildSampleAdsRows: ${error.message}`);
      return [];
    }
  }
}
